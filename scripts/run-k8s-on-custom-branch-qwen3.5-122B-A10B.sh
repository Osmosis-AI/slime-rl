#!/usr/bin/env bash
#
# Submit a single-node Qwen3.5-122B-A10B LoRA test training job to k8s.
#
# Usage:
#   bash scripts/run-k8s-on-custom-branch-qwen3.5-122B-A10B.sh --repo https://github.com/Osmosis-AI/slime-rl [--branch dev] [--dry-run]
#
# Prerequisites:
#   - kubectl configured for the target cluster
#   - Weka PVC "weka-data" in namespace "trainers" with:
#       /data/test-lora-gsm8k/gsm8k/{train,test}.parquet
#       /data/Qwen3.5-122B-A10B  (model weights; script downloads if missing)
...
JOB_NAME="local-testrun-qwen35-122b-lora-$(date +%Y%m%d-%H%M%S)"
DRY_RUN=""
...
usage() {
    cat <<EOF
Usage:
  bash scripts/run-k8s-on-custom-branch-qwen3.5-122B-A10B.sh --repo <git-repo-url> [--branch <branch>] [--dry-run]

Examples:
  bash scripts/run-k8s-on-custom-branch-qwen3.5-122B-A10B.sh --repo https://github.com/Osmosis-AI/slime-rl
  bash scripts/run-k8s-on-custom-branch-qwen3.5-122B-A10B.sh --repo https://github.com/Osmosis-AI/slime-rl --branch dev
  bash scripts/run-k8s-on-custom-branch-qwen3.5-122B-A10B.sh --repo https://github.com/Osmosis-AI/slime-rl/tree/dev
...
MODEL_PATH="/data/Qwen3.5-122B-A10B"
      ...
      - name: trainer
        ...
        - |
          ...
          if [[ -d "${MODEL_PATH}" ]] && [[ -n "\$(ls -A "${MODEL_PATH}" 2>/dev/null)" ]]; then
              echo "Found existing model weights at ${MODEL_PATH}"
          else
              echo "Downloading Qwen/Qwen3.5-122B-A10B to ${MODEL_PATH}..."
              python3 -c "from huggingface_hub import snapshot_download; snapshot_download('Qwen/Qwen3.5-122B-A10B', local_dir='${MODEL_PATH}'); print('Model download complete.')"
          fi
